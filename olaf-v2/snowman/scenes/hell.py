...
# Items
OBSTACLE_PATHS = [
    Path("image/TreeOnFire_2.png"),
    Path("image/Lava_2.png"),
    Path("image/Fire_1.png")
]
...
class HellScene:
    ...
    def start(self):
        ...
        self.bigfont = pygame.font.SysFont("Consolas", 42)

        self.bg_img = self._load_bg(HELL_BG_PATH)
        self.bg_offset = 0.0
        self.elapsed   = 0.0
        ...
        self.fx_sprites  = pygame.sprite.Group()  # split-heads etc.
        ...
        try:
            ...
            print(f"[Sound] Background music load/play failed: {e}")
    ...
    def update(self, dt: float):
        ...
        base_speed = BG_SCROLL_SPEED_BASE + (BG_SCROLL_SPEED_MAX - BG_SCROLL_SPEED_BASE) * (portion ** SPPED_CURVE if (SPPED_CURVE:=SPEED_CURVE) else SPEED_CURVE)
        speed_mult = 1.0
        if self.state["slow_timer"] > 0.0: speed_mult *= 0.6
        ...
        # collisions (skip while phased)
        if self.state["phase_timer"] <= 0.0:
            self.spawner.handle_collisions(self.player, self.state)

            # collected ice cubes this frame → add to shared ice + extend melt
            cubes = self.state.get("frame_collected_presents", 0)
            if cubes > 0:
                self.shared["ice"] += cubes
                self.state["melt"] += ICE_ADD_SECONDS * cubes
                self.state["pickup_flash"] = max(self.state["pickup_flash"], 0.12)
                if self.sounds.get("present"): self.sounds["present"].play()

            # specials → phase mode (same effects as Snowy)
            ...
            if specials:
                if any(("carrot" in n) for n in specials):
                    ...
                    self.state["phase_timer"] = 2.8
                    self.state["phase_start_flash"] = 0.30
                    self.state["active_buff"] = "carrot"
                    self._spawn_heads_on_lanes()
                if any(("moose" in n) for n in specials):
                    self.state["phase_kind"] = "moose"
                    self.state["phase_timer"] = 2.8
                    self.state["phase_start_flash"] = 0.30
                    self.state["active_buff"] = "moose"
            ...
            # phased — ignore everything; optionally clear overlaps
            collide_fn = pygame.sprite.collide_circle_ratio(1.05)
            for ob in list(self.spawner.obstacles):
                if collide_fn(self.player, ob): ob.kill()
        ...
        if self.game_over:
            if not self._played_game_over_snd:
                try:
                    ...
                    pass
            if not self._played_game_over_snd and self.sounds.get("fail"):
                self.sounds["fail"].play()
                self._played_game_over_snd = True

            ov = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA); ov.fill((20, 0, 0, 170))
            screen.blit(ov, (0, 0))
            t = self.bigfont.render("GAME OVER", True, (255, 220, 220))
            screen.blit(t, t.get_rect(center=(WIDTH//2, HEIGHT//2)))

    # ---------- helpers ----------
    def _load_bg(self, path: Path) -> pygame.Surface:
        try:
            img = pygame.image.load(str(path)).convert_alpha()
        except Exception:
            img = pygame.Surface((WIDTH, HEIGHT)); img.fill((60, 20, 20))
            return img
        iw, ih = img.get_size()
        scale = WIDTH / iw
    ...
    def _spawn_heads_on_lanes(self):
        """Spawn/refresh a head above each lane for the whole carrot phase."""
...
class _LaneHead(pygame.sprite.Sprite):
    """Split-head FX that follows carrot phase lifetime and fades out at the end."""
    def __init__(self, scene: HellScene, img: pygame.Surface, center: tuple[int, int]):
        super().__init__()
        self.scene = scene
        self.base_image = img
        self.image = img.copy()
...
class _Player(pygame.sprite.Sprite):
    def __init__(self, adjusted_lane_x, start_lane=1):
        super().__init__()
        # Use Olaf sprite if present; otherwise a simple circle
    ...
    def _load_or_make_normal(self) -> pygame.Surface:
        # Try the same Olaf sprite you used in Snowy
